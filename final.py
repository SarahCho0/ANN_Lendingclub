...
df = pl.scan_csv(
    ...
    null_values=['Loans that do not meet the credit policy'],
...
# [추가] 예전 코드에서 가져온 핵심 변수 리스트 (삭제 방지용)
SELECTED_FEATURES = [
    # 1. 기본 신용 정보 및 조회 관련
...
def get_cols_to_drop():
    drop_cols = [
        ...
        'total_pymnt_inv', 'total_pymnt', 'total_rec_prncp', 'total_rec_int', 'total_rec_late_fee',
        'recoveries', 'collection_recovery_fee', 'last_pymnt_amnt', 'last_pymnt_d', 'next_pymnt_d',
        'last_credit_pull_d', 'debt_settlement_flag', 'out_prncp', 'out_prncp_inv',
        'grade', 'sub_grade',
        'funded_amnt', 'funded_amnt_inv', 'fico_range_high', 'annual_inc', 'annual_inc_joint', 'dti_joint', 'monthly_inc',
        # 아래 변수들은 SELECTED_FEATURES에 있다면 삭제 로직에서 제외됨
        'sec_app_fico_range_low', 'sec_app_fico_range_high', 'sec_app_inq_last_6mths', 'sec_app_mort_acc',
        'sec_app_open_acc', 'sec_app_revol_util', 'sec_app_open_act_il', 'sec_app_num_rev_accts',
        'sec_app_chargeoff_within_12_mths', 'sec_app_collections_12_mths_ex_med', 'revol_bal_joint',
...
def process_pipeline(df_input, is_train=True):
    ...
    if is_train:
        ...
        q = (
            q.filter(pl.col('loan_status').is_in(target_bad + target_good))
             .with_columns(pl.col('loan_status').is_in(target_bad).cast(pl.Int8).alias('target'))
        )
    ...
    # 2. Joint Applicant 처리 및 통합 변수 생성
    q = q.with_columns([
    ...
    # [추가] 구조적 결측치 0으로 채우기 (예전 코드 반영 - Zero Fill)
    # 설명: 특정 금융 활동이 없어서 NULL인 경우 0으로 대체
    zero_fill_list = [
    ...
    if existing_zero_cols:
        q = q.with_columns([
            pl.col(c).fill_null(0).cast(pl.Float32) for c in existing_zero_cols
        ])

    # 4. 범주형 변수 수동 인코딩 (기존 코드 유지)
    q = q.with_columns([
        pl.when(pl.col('initial_list_status') == 'w').then(pl.lit(1))
          .when(pl.col('initial_list_status') == 'f').then(pl.lit(0))
          .otherwise(pl.lit(0))
          .cast(pl.Int8)
          .alias('initial_list_status')
    ])
    ...
    home_ownership_categories = q.select('home_ownership').unique().to_series().to_list()
    for category in home_ownership_categories:
        if category is not None:
            q = q.with_columns([
                (pl.col('home_ownership') == category).cast(pl.Int8).alias(f'home_ownership_{category}')
            ])

    verification_status_categories = q.select('effective_verification_status').unique().to_series().to_list()
    for category in verification_status_categories:
        if category is not None:
            q = q.with_columns([
                (pl.col('effective_verification_status') == category).cast(pl.Int8).alias(f'verification_status_{category}')
            ])
            
    # 5. 신용 카드 계좌 이진화
    q = q.with_columns([
         (pl.col('num_tl_120dpd_2m') >= 1).cast(pl.Int8).alias('num_tl_120dpd_2m'),
         (pl.col('num_tl_30dpd') >= 1).cast(pl.Int8).alias('num_tl_30dpd'),
    ])
    
    # 6. 로그화 (확장됨 - 예전 코드 반영)
    # 왜도가 높은 주요 변수들에 대해 로그 변환 수행
    log_candidates = [
        'annual_inc', 'revol_util', 'int_rate', # 기존 코드 변수
        'tot_cur_bal', 'tot_hi_cred_lim', 'total_bal_ex_mort', 'avg_cur_bal', # 예전 코드 추가
        'total_bc_limit', 'total_il_high_credit_limit', 'total_rev_hi_lim',
        'inq_last_12m', 'bc_open_to_buy', 'delinq_amnt'
    ]
    
    # 존재하는 컬럼에 대해서만 로그 변환 (log1p 적용하여 0값 처리)
    cols_to_log = [c for c in log_candidates if c in q.schema]
    if cols_to_log:
        q = q.with_columns([
            pl.when(pl.col(c) > 0)
              .then(pl.col(c).log1p()) # log 대신 log1p 사용 (안전성)
              .otherwise(pl.lit(0.0))
              .cast(pl.Float32)
              .alias(f'log_{c}') # 이름 변경: log_annual_inc 등
    ...
    # 8. Month 변수 결측치 처리 (예전 코드 우선 적용 + 기존 코드 Fallback)
    schema = q.schema
    
    # [수정] 예전 코드의 mths_fill_plan 적용
    mths_fill_plan = {
    ...
    for col_name, fill_val in mths_fill_plan.items():
        if col_name in schema:
            q = q.with_columns([
                pl.col(col_name).is_null().cast(pl.Int8).alias(f'is_never_{col_name}'),
                pl.col(col_name).fill_null(fill_val).cast(pl.Float32)
            ])
            
    # 그 외 mths_ 변수들은 기존 방식(Quantile) 적용
    mths_cols = [c for c in schema.names() if 'mths_since' in c and c not in mths_fill_plan]
    ...
    # 9. 컬럼 삭제 (SELECTED_FEATURES 보호 로직 추가)
    drop_candidates = get_cols_to_drop()
    drop_candidates.extend(['application_type', 'tax_liens', 'pub_rec', 'emp_length',
                           'annual_inc_joint', 'annual_inc', 'dti', 'dti_joint', 
                           'earliest_cr_line_parsed', 'earliest_cr_line', 'home_ownership', 'purpose'])
    
    # [중요 변경] FINANCE_COLS 뿐만 아니라 SELECTED_FEATURES에 있는 변수도 삭제하지 않음
    real_cols_to_drop = [
        c for c in drop_candidates 
        if c not in FINANCE_COLS and c not in SELECTED_FEATURES
    ]
...
def calculate_actual_irr(df_pl):
    ...
    if "int_rate" in df.columns:
        ...
        if charged_off_mask.any():
            df.loc[charged_off_mask, "actual_irr"] = df.loc[charged_off_mask].apply(
                lambda row: np.clip(
                    row["actual_irr"], 
                    -1.0, 
                    int_rate_decimal.loc[row.name] if pd.notna(int_rate_decimal.loc[row.name]) else 0.6
        ...
        df["actual_irr"] = df["actual_irr"].clip(lower=-1.0, upper=0.6)
...
# 결측치를 평균으로 대체 (숫자 컬럼만 대상)
X_rf = X_rf.fillna(X_rf.select_dtypes(include=['number']).mean())
...
rf = RandomForestClassifier(
    ...
    n_jobs=2               # macOS 호환성: 2~4 권장 (n_jobs=-1은 잠금 문제 유발)
)
...
if len(features_with_nulls) > 0:
    ...
    for feat, count in features_with_nulls.items():
        ...
        X.loc[:, feat] = X[feat].fillna(mean_val)
...
# ============================================================================
# 📌 목표: Train 60% : Validation 20% : Test 20%로 계층화 분할

# ============================================================================
# ⚙️ [조절 포인트 2] 데이터 분할 비율
# ============================================================================
# 🔧 조절 방법:
#    현재: Train 60% : Validation 20% : Test 20%
#    변경하려면:
#    - test_size=0.4 → 다른 비율로 변경 (예: 0.3 → 70:30 분할)
#    - 두 번째 test_size=0.5 → 40%를 50:50으로 분할
#    
#    예시: Train 70 : Val 15 : Test 15
#    - 첫 번째: test_size=0.3 (Train 70%, Temp 30%)
#    - 두 번째: test_size=0.5 (Val 15%, Test 15%)

print(f"\n📊 데이터 분할 중...")

# Train(60%) vs Temp(40%) 분할
X_train, X_temp, y_train, y_temp, w_train, w_temp, meta_train, meta_temp = train_test_split(
    X_processed, y, weights, meta,
    test_size=0.4,                  # 🔧 0.4 = 40%를 Temp로 사용 (Train은 60%)
    random_state=42,
    stratify=y
)

# Temp를 Validation(20%) vs Test(20%)로 분할
X_val, X_test, y_val, y_test, w_val, w_test, meta_val, meta_test = train_test_split(
    X_temp, y_temp, w_temp, meta_temp,
    test_size=0.5,                  # 🔧 0.5 = 40%의 50% (Validation과 Test 각각 20%)
    random_state=42,
    stratify=y_temp
)

print(f"\n📊 데이터 분할 완료:")
print(f"  Train: {len(X_train):,}건 ({len(X_train)/len(X_processed)*100:.1f}%)")
print(f"  Val:   {len(X_val):,}건 ({len(X_val)/len(X_processed)*100:.1f}%)")
print(f"  Test:  {len(X_test):,}건 ({len(X_test)/len(X_processed)*100:.1f}%)")
...
early_stopping = keras.callbacks.EarlyStopping(
    monitor='val_loss',
    patience=10,                    # 🔧 개선 없는 epoch 수 (기본값 10, 범위: 5~20)
...
history = model.fit(
    ...
    epochs=50,                      # 🔧 최대 반복 횟수 (기본값 50, 범위: 30~150)
...
def calculate_sharpe_ratio_topk(y_pred_prob, meta_data, df_full, top_k):
    ...
    # 가중 평균 초과 수익률
    weighted_excess = np.sum(excess_returns * portfolio_weights)
    
    # 가중 평균 수익률
    weighted_portfolio_return = np.sum((excess_returns + risk_free_rates) * portfolio_weights)
    
    # 가중 분산 계산
    weighted_variance = np.sum(
        portfolio_weights * ((excess_returns + risk_free_rates) - weighted_portfolio_return) ** 2
    )
    ...
    if weighted_std < MIN_STD:
        ...
        if weighted_excess > 0:
            ...
            sharpe_ratio = 0.0
        return sharpe_ratio, weighted_portfolio_return
    
    # Weighted Sharpe Ratio
    sharpe_ratio = weighted_excess / weighted_std
...
print(f"\n{'비율':>8} | {'K (개수)':>10} | {'Weighted Sharpe':>15}")
print("-" * 40)
...
print(f"🏆 최적 비율: {best_ratio:.1f}% ({best_k:,}개) (Weighted Sharpe: {best_sharpe:.4f})")
print(f"   Weighted Portfolio Return: {best_port_return:.4f}")
print("="*80)

# STEP 7-3. 최고 성능 K 비율 Top 10 출력
# ============================================================================
print(f"\n📊 Top 10 최고 성능 비율:")
print(f"{'순위':>4} | {'비율':>8} | {'K (개수)':>10} | {'Weighted Sharpe':>15}")
print("-" * 50)
...
print(f"  Weighted Sharpe: {test_sharpe:.4f}")
print(f"  Weighted Portfolio Return: {test_port_return:.4f}")
...
def get_benchmark_performance(meta_data, df_full):
    ...
    sharpe_ratio = weighted_excess / weighted_std  # ← 이건 새로 추가
    
    return sharpe_ratio, weighted_portfolio_return  # ← 여기만 수정 (원래 return weighted_excess / weighted_std)
...
print(f"\n📊 벤치마크 (모든 대출 승인) 성과:")
print(f"  Validation Weighted Sharpe: {benchmark_val_sharpe:.4f}")
print(f"  Validation Weighted Return: {benchmark_val_return:.4f}")
print(f"  Test Weighted Sharpe:       {benchmark_test_sharpe:.4f}")
print(f"  Test Weighted Return:       {benchmark_test_return:.4f}")
...
print(f"📌 평가 방식: Weighted Sharpe (대출 금액 비중 반영 )")
print(f"")
val_port_return = return_results[best_k]
print(f"📈 Validation 성과:")
print(f"   최적 K: {best_k:,}개 ({(best_k/len(y_val_pred_prob)*100):.1f}%)")
print(f"   Top-K Weighted Sharpe:    {best_sharpe:.4f}")
print(f"   Top-K Weighted Return:    {val_port_return:.4f}")
print(f"   Benchmark Weighted Sharpe: {benchmark_val_sharpe:.4f}")
...
print(f"")
print(f"📈 Test 성과:")
print(f"   Top-K Weighted Sharpe:    {test_sharpe:.4f}")
print(f"   Top-K Weighted Return:    {test_port_return:.4f}")
print(f"   Benchmark Weighted Sharpe: {benchmark_test_sharpe:.4f}")
...
with open(report_file, 'w', encoding='utf-8') as f:
    ...
    f.write(f"  Weighted Portfolio Return: {test_port_return:.4f}\n")  # ← 이 줄 추가
...
plt.xlabel('Weighted Sharpe Ratio', fontsize=12)
